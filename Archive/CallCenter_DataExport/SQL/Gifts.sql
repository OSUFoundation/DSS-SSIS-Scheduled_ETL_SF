...
SELECT DISTINCT
    ...
    --,CGPRAMOUNT        
    ,CASE 
        WHEN gi.InstallmentStatus = 'Write-off' AND CGPRAMOUNT = 0 AND  FC.FUNDID IS NULL THEN CONCAT('Pledge-Write-Off- ','DO NOT SOLICIT- ',CPR.FUNDDESCRIPTION)       
        WHEN gi.InstallmentStatus = 'Write-off' AND CGPRAMOUNT = 0 THEN CONCAT('Pledge-Write-Off- ',CPR.FUNDDESCRIPTION)
