...
SELECT
	...
	,CASE  
        WHEN S_COLLEGE_DESC LIKE '%Arts%'	        THEN 'ARTSCI'
		WHEN S_COLLEGE_DESC LIKE '%Automotive%'	    THEN 'AUTO'
		WHEN S_COLLEGE_DESC LIKE '%Constr%'		    THEN 'CONSTECH'
		WHEN S_COLLEGE_DESC LIKE '%Culinary%'	    THEN 'CULART'
		WHEN S_COLLEGE_DESC LIKE '%Energy%'	        THEN 'ENERTECH'
		WHEN S_COLLEGE_DESC LIKE '%Engineering%'	THEN 'ENGTECH'
		WHEN S_COLLEGE_DESC LIKE '%HEVi%'	        THEN 'HEVI'
		WHEN S_COLLEGE_DESC LIKE '%Info%'	        THEN 'IT'
		WHEN S_COLLEGE_DESC LIKE '%Nurs%'	        THEN 'NURSE'
		WHEN S_COLLEGE_DESC LIKE '%Visual%'	        THEN 'VISCOMM'
        
		END																AS MajorOrganization1
	,CASE
        WHEN S_COLLEGE_DESC LIKE '%Arts%'	        THEN '(IT) Arts & Science'
		WHEN S_COLLEGE_DESC LIKE '%Automotive%'	    THEN '(IT) Automotive'
		WHEN S_COLLEGE_DESC LIKE '%Constr%'		    THEN '(IT) Construction Technology'
		WHEN S_COLLEGE_DESC LIKE '%Culinary%'	    THEN '(IT) Culinary Arts'
		WHEN S_COLLEGE_DESC LIKE '%Energy%'	        THEN '(IT) Energy Technologies'
		WHEN S_COLLEGE_DESC LIKE '%Engineering%'	THEN '(IT) Engineering Technology'
		WHEN S_COLLEGE_DESC LIKE '%HEVi%'	        THEN '(IT) HEVi'
		WHEN S_COLLEGE_DESC LIKE '%Info%'	        THEN '(IT) IT'
		WHEN S_COLLEGE_DESC LIKE '%Nurs%'	        THEN '(IT) Nursing & Health Science'
		WHEN S_COLLEGE_DESC LIKE '%Visual%'	        THEN '(IT) Visual Communications'
