...
SELECT t.*
FROM "OSUADV_PROD"."BI"."DNRCNCT_EDUCATION" t
		INNER JOIN "OSUADV_PROD"."BI"."DNRCNCT_InclLst_minus_ExclLst" IME ON IME.CONSTITUENTSYSTEMID = t.CONSTITUENTSYSTEMID
;
...
SELECT t.*
FROM "OSUADV_PROD"."BI"."DNRCNCT_PRODUCTIONGIFTS" t
		INNER JOIN "OSUADV_PROD"."BI"."DNRCNCT_InclLst_minus_ExclLst" IME ON IME.CONSTITUENTSYSTEMID = t.CONSTITUENTSYSTEMID
;
...
SELECT t.*
FROM "OSUADV_PROD"."BI"."DNRCNCT_RECEIPTEDGIFTS" t
		INNER JOIN "OSUADV_PROD"."BI"."DNRCNCT_InclLst_minus_ExclLst" IME ON IME.CONSTITUENTSYSTEMID = t.CONSTITUENTSYSTEMID
;
	...
	(
		...
		FROM
			(
				...
				, SCHOOLTYPE
				, GRADYEAR
				, ROW_NUMBER() OVER( PARTITION BY CONSTITUENTSYSTEMID ORDER BY GRADYEAR DESC) AS Seq
...
CREATE OR REPLACE TEMPORARY TABLE tmp_SegmentCode 
    (
        segmentName     varchar(100)
        ,segmentCode    varchar(100)
    );

INSERT INTO tmp_SegmentCode
...
union select 'CEAT 4+ 10-20'segmentName									,'CEAT_4YR_10-20' segmentCode
...
union select 'CEHS 1 Yr Lapsed New and Reactivated'segmentName			,'CEHS_1YR_NR' segmentCode
...
union select 'DASNR 4+ 10-20'segmentName								,'DASNR_4YR_10-20' segmentCode
...
union select 'SSB 2-3 Yr Lapsed'segmentName								,'SSB_2-3YR' segmentCode
...
, CASE WHEN SC.segmentName = SegmentData.SegmentName THEN SC.segmentCode END AS segmentCode
, 'FY21 - Fall' as segmentGroup
    ...
    "OSUADV_PROD"."RE"."CONSTITUENT" AS C
        INNER JOIN "OSUADV_PROD"."BI"."DNRCNCT_Household" h ON h.PRIMARYPROSPECTID = C.CONSTITUENTID					
							...
							INNER JOIN
															  	...
															  	"OSUADV_PROD"."BI"."DNRCNCT_InclLst_minus_ExclLst" AS IL 
																						...
																						LEFT OUTER JOIN 
																									(																						
																										...
																										WHERE 
																											GRADYEAR BETWEEN 1980 AND 2010	
																											AND SchoolType IN ('OSU-CHS', 'CVHS', 'CVM', 'OSUIT', 'OSU-OKC')
										...
										) AS SegmentData ON C.CONSTITUENTSYSTEMID = SegmentData.CONSTITUENTSYSTEMID
